...
export function GamePage(): React.JSX.Element {
	...
	const deckNames: string[] = location.pathname
		.split("/")[2]
		?.split(",")
	...
	const [translation, setTranslation] = useState<number>(
		() => (1490 * scaleFactor + window.innerWidth) / 2
	);
	useEffect(function () {
		window.addEventListener("resize", windowResizeHandler);
		return () => window.removeEventListener("resize", windowResizeHandler);
		function windowResizeHandler() {
			let newScaleFactor: number = Math.min(
				(window.innerHeight * 0.7) / 1040,
	...
	return (
		<IonPage>
			...
			<IonModal
				...
				onDidDismiss={() => setTextModalOpen(false)}
			>
				<Modal
					title={card?.name}
					closeFn={() => setTextModalOpen(false)}
				>
					<Card card={card} />
				</Modal>
			</IonModal>
			<IonModal isOpen={optionsModalOpen} backdropDismiss={false}>
				<Modal
					title="Options"
					closeFn={() => setOptionsModalOpen(false)}
				>
