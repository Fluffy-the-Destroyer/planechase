...
export function rollPlanarDie(chaoticAether: number = 0): string {
  ...
  switch (Math.floor(6 * Math.random())) {
    ...
    default:
      return chaoticAether > 0 ? Array(chaoticAether).fill("C").join("") : "B";
  }
}

type generator<T = any, U = any, V = void> = (this: planarDeck) => Generator<T, U, V>;
...
export class planarDeck {
  ...
  shuffle(this: planarDeck): void {
    ...
    /**Holds random number */
    let r: number;
    for (let unshuffled: number = this.planarLibrary.length; unshuffled > 1; ) {
